...
const NOTES = [
  ...
  "- `Set-Content`/`Add-Content` default to the system ANSI codepage when writing a",
  "  file, not UTF-8 — a file written with Chinese text through either without an",
  "  explicit encoding can come out corrupted on disk, not just on screen. Pass",
  "  `-Encoding utf8` explicitly, or write the file from Python instead.",
  "- Forwarding a command with nested quotes through SSH from PowerShell (e.g.",
  "  `ssh host \"sudo python3 -c \\\"...\\\"\"`) can silently hang for minutes instead of",
  "  failing outright once the quoting gets more than one level deep. Past a single",
  "  level of nesting, write the command to a local script file and `scp` it over",
  "  instead of trying to get the escaping right inline.",
  "",
  "Filesystem:",
  ...
  "- `NUL`/`CON`/`PRN`/`AUX`/`COM1`-`COM9`/`LPT1`-`LPT9` are reserved device names in",
  "  every directory. Discarding output with Unix-style `> /dev/null` doesn't hit the",
  "  null device here — it can create a literal, barely-deletable file named `nul` in",
  "  the working directory instead. Use PowerShell's `$null` (`> $null`, `| Out-Null`).",
  ...
  "  letter's colon looks like another `host:container` separator. The mount silently fails,",
  "  the container still reports healthy, and it just runs on its compiled-in defaults. If a",
  "  bind-mounted config doesn't seem to take effect, `docker exec` in and `cat` the file",
  "  before assuming the application code is wrong.",
  "",
  "Cross-platform builds:",
  "- npm packages with native (.node) bindings, built here, produce Windows binaries.",
  "  If the target is a Linux server, those .node files are the wrong platform and will",
  "  crash there even though `npm install` and the build both reported success locally.",
  "  Before shipping, check with something like `find . -name '*.node' | xargs file` for",
  "  anything that says \"PE32+\" or \"MS Windows\", and exclude/rebuild those on the target.",
  "",
